...
class GptImage2Service(BaseService):
    ...
    async def edit_image(
        ...
        async def task() -> ImageResult:
            return await self._edit_image_internal(
    ...
    async def _edit_image_internal(
        ...
        """立即编辑一张图片（图生图）。"""
        if not prompt.strip():
            return False, "编辑指令不能为空", None
        if not source_image_base64.strip():
            return False, "来源图片不能为空", None
            ...
            for attempt in range(self.max_retries + 1):
                try:
                    ...
                    logger.info(
                        ...
                        f"url={url}, timeout={self.timeout}s"
                    )
